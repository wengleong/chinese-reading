...
// SpeechRecognition (zh-CN) runs in parallel to capture a reading transcript
// for AI scoring. Calls onComplete({ transcript, story }) when recording stops.
...
export function renderRecorder({ root, getCurrentStory, onSaved, onActiveChange, onComplete }) {
  ...
  note.textContent = "Recordings stay on this device only. Ask a parent or teacher to review before sharing.";
  ...
  async function start() {
    ...
    if (!navigator.mediaDevices?.getUserMedia) { alert("This browser does not support media recording."); return; }
    ...
    if (SR) {
      try {
        ...
        recognition.onerror = () => {};  // silent — scoring still works with empty transcript
        recognition.start();
    ...
    recorder.onstop = async () => {
      ...
      try {
        ...
        console.warn("Failed to save recording:", err.message);
      }
